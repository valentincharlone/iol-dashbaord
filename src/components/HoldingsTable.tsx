...
function fmtMoney(n: number) { return "$" + Math.round(n).toLocaleString("es-AR"); }
function fmtPct(n: number)   { return (n >= 0 ? "+" : "") + Math.abs(n).toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + "%"; }
...
export function HoldingsTable({ posiciones, totalValuacion }: Props) {
  ...
  const thSort = (key: SortBy, label: string, extra = "") => (
    ...
    >
      {label}
      <span className={`ml-0.5 ${sortBy === key ? "opacity-100" : "opacity-30"}`}>
    ...
    <div className="bg-white rounded-card shadow-sm overflow-clip">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border-light">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] text-text3">{posiciones.length} posiciones</span>
        </div>
            ...
            {sorted.map((p, i) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] font-semibold px-1 py-px rounded-[3px] tracking-[0.3px]"
                            style={{ background: badge.bg, color: badge.text }}>
                            {tipoLabel(p.tipo)}
                          </span>
                        </div>
                        <div className="text-[11px] text-text3 mt-px">{p.nombre}</div>
                      </div>
                    </div>
                  </td>

                  {col("cantidad")  && <td className={tdCls}>{p.cantidad.toLocaleString("es-AR")}</td>}
                  {col("precio")    && <td className={tdCls}>{fmtMoney(p.precioActual)}</td>}
                  {col("costo")     && <td className={`${tdCls} text-text3`}>{fmtMoney(p.ppc)}</td>}
                  {col("valuacion") && <td className={`${tdCls} font-bold`}>{fmtMoney(p.valuacion)}</td>}

                  {col("pnl") && (
                    <td className={tdCls}>
                      <div className={`font-semibold ${isPos ? "text-profit" : "text-loss"}`}>{fmtPct(p.pnlPorcentaje)}</div>
                      <div className={`text-[10px] opacity-65 ${isPos ? "text-profit" : "text-loss"}`}>
                  ...
                  {col("hoy") && (
                    <td className={tdCls}>
                      <span className={`font-semibold text-[12px] ${hoyPos ? "text-profit" : "text-loss"}`}>
                  ...
                  {col("peso") && (
                    <td className={`${tdCls} pr-5`}>
                      <div className="font-medium text-[12px]">{peso.toFixed(1)}%</div>
                      <div className="h-[3px] rounded-sm bg-brand-muted mt-0.5 w-14">
                        <div
                          ...
                          style={{ width: (maxWeight > 0 ? (peso / maxWeight) * 100 : 0) + "%" }}
                        />
                      </div>
                    </td>
                  )}
