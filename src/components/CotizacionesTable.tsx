...
const TH: React.CSSProperties = {
  ...
  textAlign: "right", whiteSpace: "nowrap", userSelect: "none", cursor: "pointer",
};

export function CotizacionesTable({ items }: Props) {
  const [sort, setSort] = useState<{ key: SortKey; dir: SortDir }>({ key: "variacion", dir: "desc" });
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (i) => !q || i.ticker.toLowerCase().includes(q) || i.nombre.toLowerCase().includes(q)
    );
  }, [items, search]);
    ...
    <div style={{ background: "white", borderRadius: 14, border: "1px solid var(--border-light)",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 15, fontWeight: 600, color: "var(--text-1)" }}>Cotizaciones</span>
          <span style={{ fontSize: 12, color: "var(--text-3)" }}>{sorted.length} instrumentos</span>
        </div>
        <input
          type="text"
          ...
          style={{
            ...
            fontFamily: "inherit", color: "var(--text-1)", width: 220,
          }}
        />
      </div>
            ...
            <tr>
              {COLUMNS.map((col) => (
                <th
                  ...
                  style={{ ...TH, textAlign: col.align ?? "right", paddingLeft: col.align === "left" ? 20 : 12 }}
                >
                ...
                >
                  <td style={{ ...tdBase, textAlign: "left", paddingLeft: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontWeight: 700, fontSize: 13, color: "var(--text-1)" }}>{item.ticker}</span>
                  ...
                  <td style={{ ...tdBase, fontWeight: 700 }}>{fmtMoney(item.precio)}</td>
                  <td style={tdBase}>
                    <span style={{ fontWeight: 600, color: pos ? "#059669" : "#EF4444" }}>
                      {fmtPct(item.variacionPorcentual)}
                    </span>
                  </td>
                  <td style={{ ...tdBase, color: "var(--text-2)" }}>{fmtMoney(item.apertura)}</td>
                  <td style={{ ...tdBase, color: "#059669" }}>{fmtMoney(item.maximo)}</td>
                  <td style={{ ...tdBase, color: "#EF4444" }}>{fmtMoney(item.minimo)}</td>
                  <td style={{ ...tdBase, color: "var(--text-2)" }}>{fmtVol(item.volumenNominal)}</td>
                  <td style={{ ...tdBase, paddingRight: 20, color: "var(--text-2)" }}>{fmtMoney(item.montoOperado)}</td>
                </tr>
              );
            })}
            {sorted.length === 0 && (
              <tr>
                <td colSpan={8} style={{ padding: "32px", textAlign: "center", fontSize: 13, color: "var(--text-3)" }}>
